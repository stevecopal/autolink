...
from django.db.models import Q, Count, Sum, F, FloatField, ExpressionWrapper, Prefetch, Avg, Case, When, IntegerField
...
def garage_list_view(request):
    ...
    if latitude and longitude and radius:
        try:
            ...
            garages_with_coords = garages.exclude(latitude__isnull=True, longitude__isnull=True)

            nearby_ids = []
            for garage in garages_with_coords:
                distance = _haversine_distance(lat, lng, garage.latitude, garage.longitude)
...
def garage_list_api(request):
    ...
    return JsonResponse({
        ...
        'pagination': {
            ...
            'next_page': garages_page.next_page_number() if garages_page.has_next() else None,
            'previous_page': garages_page.previous_page_number() if garages_page.has_previous() else None,
        },
        'filters': {
            'city': city,
...
def garage_detail_view(request, slug):
    garage = get_object_or_404(
        Garage.objects.select_related('owner', 'city', 'neighborhood').prefetch_related(
            ...
            Prefetch('photos', queryset=GaragePhoto.objects.order_by('-is_primary', '-created_at')),
            Prefetch('brands', queryset=GarageBrand.objects.select_related('brand').order_by('brand__name')),
            Prefetch('reviews', queryset=__import__('reviews.models', fromlist=['Review']).Review.objects.filter(is_hidden=False).select_related('user').order_by('-created_at')),
        ),
        slug=slug,
        is_active=True
    )
    ...
    if request.headers.get('HX-Request'):
        return render(request, 'public/pages/garages/partials/garage_detail_content.html', context)
...
def garage_dashboard_view(request):
    ...
    products = Part.objects.filter(garage=garage, is_active=True).select_related('category', 'brand')
    recent_reviews = Review.objects.filter(garage=garage, is_hidden=False).select_related('user')[:5]

    total_sales = Order.objects.filter(garage=garage, status__in=['PAID', 'PROCESSING', 'READY', 'PICKED_UP', 'DELIVERED']).aggregate(
        total=Sum('total')
    )['total'] or 0
