...
def approve_garage(garage, admin_user=None):
    ...
    garage.save(update_fields=['verification_status', 'rejection_reason', 'updated_at'])

    owner = garage.owner
    promoted = False

    if owner.role != User.Role.CLIENT:
        owner.role = User.Role.CLIENT
        owner.save(update_fields=['role'])
        promoted = True
        logger.info(
            "Utilisateur %s promu en CLIENT (garage %s approuvé par %s)",
            owner.username, garage.name,
    ...
    return {
        'promoted': promoted,
        'message': (
            f"Garage '{garage.name}' approuvé. "
            f"{owner.username} promu en CLIENT." if promoted
...
def reject_garage(garage, reason='', admin_user=None):
    ...
    has_approved = owner.garages.filter(
        verification_status=garage.VerificationStatus.APPROVED
    ).exclude(pk=garage.pk).exists()
    ...
    return {
        'demoted': demoted,
        'message': (
            f"Garage '{garage.name}' rejeté. "
            f"{owner.username} rétrogradé en USER." if demoted
...
def suspend_garage(garage, admin_user=None):
    ...
    garage.save(update_fields=['verification_status', 'updated_at'])
...
def update_garage_availability(garage, availability_status, message=''):
    ...
    if garage.verification_status != Garage.VerificationStatus.APPROVED:
        return {
            'success': False,
    ...
    garage.save(update_fields=['availability_status', 'availability_message', 'updated_at'])
