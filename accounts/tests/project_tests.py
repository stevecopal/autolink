# admin/tests/project_tests.py
"""
Project-level tests for the admin application.
Kept in one file to avoid long, hard-to-correct monstrous test files.
"""
...
class AdminDashboardRedirectTest(TestCase):
    def test_anonymous_user_is_redirected_from_admin_pages(self):
        response = self.client.get(reverse("administration:dashboard"), follow=False)
    ...
    def test_admin_user_can_access_admin_dashboard(self):
        user = User.objects.create_user(username="admin", role="ADMIN")
